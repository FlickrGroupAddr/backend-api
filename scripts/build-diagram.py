  ...
  <diagram id="fga-architecture" name="FlickrGroupAddr Architecture">
    <mxGraphModel dx="1422" dy="900" grid="0" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="{AUTHORED.width}" pageHeight="{AUTHORED.height}" math="0" shadow="0">
      <root>
        ...
        <!-- Card #0005. The canvas draws the Lightroom Classic journey, and ADR-18 says
             the browser is an equally first-class client. Drawing both as line-of-flow
             made the picture a mesh, so Terry asked for prose instead: "Maybe small note
             in some empty corner that user can drive via browser but it's simpler subset
             than plugin." He approved this corner: "I am good with that."
             It sits directly under the Browser tile and shares its INK box exactly: the
             glyph is an image with no stroke, so its ink runs 30.0 to 213.0, while this
             panel carries a 1-unit stroke that paints half outside its geometry. x=30.5
             with width 182 therefore lands the same 30.0 to 213.0, and the geometric
             center still falls on 121.5 with the rest of the Lightroom spine.
             **Geometric x=30 was the first attempt and it FAILED the build**: the ink
             reached 29.50, which moved the canvas ink box and left the authored sheet
             needing a 0.51 move it is asserted never to need. -->

        <mxCell id="browsernote" value="&lt;b&gt;Browser client&lt;/b&gt;&lt;div style=&quot;margin-left:26px;font-size:13px&quot;&gt;The same API serves a browser. It does less than the plug-in: no catalog, and no photo IDs out of Lightroom. ADR-18 ships both in lockstep.&lt;/div&gt;" style="rounded=0;whiteSpace=wrap;html=1;align=left;verticalAlign=top;fillColor=#FFFFFF;strokeColor=#B0B0B0;fontSize=14;spacingLeft=10;spacingTop=8;spacingRight=8;" vertex="1" parent="1">
          <mxGeometry x="30.5" y="706" width="159" height="170" as="geometry" />
        </mxCell>
        ...
        <mxCell id="e11" value="" style="edgeStyle=orthogonalEdgeStyle;rounded=0;html=1;endArrow=block;endFill=1;startArrow=block;startFill=1;endSize=3;startSize=3;strokeWidth=3;strokeColor=#1A1A1A;fontSize=14;labelBackgroundColor=#FFFFFF;exitX=0.928962;exitY=0.984615;exitDx=0;exitDy=0;exitPerimeter=0;entryX=0.5;entryY=1;entryDx=0;entryDy=0;" edge="1" parent="1" source="users" target="flickrapi">
          <!-- **The plumb leg moved from x=121.5 to x=200 on 2026-08-19, card #0005.**
               It used to drop from the Browser's bottom CENTER, straight through the only
               region big enough to hold `browsernote`. The empty-space scan that chose
               that corner ignored edges, said so, and was believed anyway; a fully green
               build then drew the note with an arrow through it, because nothing asserts
               a tile clears an edge.
               **Exiting bottom-RIGHT is also the better route on its own terms.** The
               target `flickrapi` is far to the right, so the old exit went left-of-travel
               before turning. exitX = (200 - 30) / 183 = 0.928962 keeps the leg on the
               tile it leaves. -->
          <mxGeometry relative="1" as="geometry">
            <Array as="points">
              <mxPoint x="200" y="1053.5" />
...
# before editing it.**
# **It went 20 -> 21 -> 20 within one hour on 2026-08-19, and the round trip is the
# useful part.** `browsernote` was first drawn 182 wide, centered on the Lightroom spine
# at x=121.5, which added an `n6` pair. The note then had to narrow to 159 so `e11`'s
# plumb leg could clear it, and the centerline moved to 110 -- so the pair went away
# again.
#
# **The check caught BOTH directions**, which is exactly what a pinned count is for. A
# suite that only noticed the gain would have let the silent loss through.
...
PROSE_PANELS = {"journey", "key", "justification", "browsernote"}
