...
| 11 | `not_started` | `Spike: can Trello be the work log?` | **Terry, 2026-08-18, cleared hot.** He pays for Trello and likes it, and is **open to Trello becoming the single source of truth**. Investigate in his stated order of preference: (1) a Claude skill or MCP server for Trello, (2) a Python SDK published by Trello itself, (3) a community-maintained Python library, (4) failing all of those, whether Trello exposes a public REST or GraphQL API. **Record the survey either way** -- "there wasn't one" needs evidence. **Note the shape this has to fit**: Claude currently owns `docs/WORK-LOG.md` exclusively and Terry never edits it, which is what makes a read-only viewer sufficient. Trello is editable by him, so it would make the log BIDIRECTIONAL and the sync contract would need rewriting rather than reusing |
