...
| **A pool with `throttle.mode: "disabled"` answers `groups.pools.add` with code 11** | **The first real add FGA ever made, 2026-08-13.** Photo `54913467650` into `2389839@N23` ("in explore"), chosen because it is `mode: disabled` **and** `poolModerated: false`, so no volunteer could be reached whatever happened. Flickr answered **11**, ADR-07 classified it terminal, and the row resolved `failed` with `flickr_code: 11` and **no `moderated_pairs` record** — verified by reading the SQLite file directly, not only the API's own reply. **This was a genuine open question**: `throttle.mode: "disabled"` and error 11 "group pool is disabled" are different fields that share a word, and code 5 (retryable) was the plausible alternative. **The whole add path is now proven end to end against real Flickr.** | 2026-08-13 |
...
| `throttle.mode` takes five values, not three | Live `groups.getInfo` across **all 372** of the owner's real groups, 2026-08-13: **`none`** 170, **`day`** 167, **`week`** 17, **`month`** 13, **`disabled`** 5 (always paired with `count: 0`). The documentation shows only `month`, which is the **fourth**-commonest value here. **`disabled` is the operationally important one** — those pools accept no adds at all, so FGA **SHOULD** skip them rather than spend an attempt discovering it. **They are not exclusively moderated or invite-only**: of the five, three are moderated and two are not, and only one of the five is invite-only. A rule keyed on invite-only would have missed four of them. | 2026-08-13 |
...
| One account can belong to hundreds of groups, and the number moves | The owner was in **330** early on 2026-08-13 and **372** later the same day. Recorded because it invalidated a design assumption rather than as trivia: an endpoint that made one call per group took **53 seconds** and returned **979 KB**. Any per-group work **MUST** be sized against hundreds, not a handful, **and MUST NOT cache the count** — it changed by 42 within a day. A full per-group `getInfo` sweep of all 372 takes roughly **50 seconds** at ~130 ms per call, which is the real cost of any feature that needs throttle data for every group. | 2026-08-13 |
...
- **Flickr's per-group add limits are now quantified, and two changes follow that are not yet
  ...
  **Downgraded from a correctness concern to a pure optimization on 2026-08-13**, because a live
  add into a `disabled` pool was measured and Flickr answers **code 11**, which ADR-07 already
  classifies terminal. The request therefore resolves and leaves its queue on the first attempt
