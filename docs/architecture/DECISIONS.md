...
**Verification: Inspection for Rules 1 and 2, Test for Rule 3.** Where a value is born is read
rather than run. **Rule 3 is a real gate** — `scripts/lua-imports.py`, wired into `npm run check`,
which self-tests in both polarities and refuses to pass on an empty directory. **No mutation**, for
the same reason ADR-13 and ADR-21 carry `—`: the Lightroom client is Lua and
`scripts/mutation-check.py` runs the Vitest suite, which cannot see it.

**THREE INDEPENDENT RULES. Each stands if the others are wrong, and they MUST NOT be collapsed into
one.**
...
### Rule 3 — Import nothing the SDK does not document, and the gate enforces it
...
general principle gets reasoned around.

#### Three refinements that the obvious version of this rule gets wrong

| Naive form | Why it breaks | What it says instead |
|---|---|---|
| *"documented in the most recent SDK"* | **The target moves without us.** This project pins `LrSdkVersion = 15.3` while Terry runs LrC 15.5, so "most recent" could change the moment Adobe publishes and **retroactively** make committed code non-compliant | The SDK version **this project pins**, which moves deliberately |
| *"documented in the SDK"*, unqualified | **Taken literally it bans the Lua language.** `string.format`, `io.open`, `os.time` and `pairs` appear in no Adobe reference because they are Lua 5.1 stdlib | Adobe's surface only. The Lua standard library is explicitly out of scope |
| Sample code counts as documentation | Adobe's shipped Flickr sample declares `LrSdkVersion = 3.0` and has never been modernized | **The API Reference and Manual are the contract. Samples are illustrative** |

#### The gate reads a committed list, and cross-checks it when it can

**`vendor/`'s SDK archive is gitignored — it is Adobe's to license and this repository is public —
so a fresh clone has no SDK.** A gate that needed the archive would be unrunnable on the machine
most likely to need it. So `scripts/lrc-sdk-modules.json` holds the 56 module names, generated by
`--regenerate` and committed.

**When the archive IS present the two are cross-checked**, which closes the hole that design would
otherwise open: a stale JSON after an SDK bump, silently approving a namespace the new SDK dropped.
...
**`import(name)` with a variable is invisible to a literal-string check.** A namespace list in a
table plus a loop defeats the entire gate.

**That is not hypothetical. The first run of `lua-imports.py` reported `0 undocumented` against
`EntropyProbe.lua`, a file whose entire purpose is importing `LrUUID`.** It reads its candidates out
of a table, so the pattern saw nothing and the clean line read as coverage.

**So a non-literal argument is reported as `UNVERIFIABLE` and requires the same explicit
exemption.** A gate that silently ignores what it cannot read is worse than no gate.
...
**Rule 3 — importing anything undocumented — reopens only if the SDK documents it.** That is the
same test as Rule 2's first condition, because it is the same objection. **Narrowing the rule to a
blacklist MUST NOT count as reopening it**: a list of known-bad namespaces is the shape this rule
replaced, and it fails on the next one nobody has found yet.
