...
## ADR-23 — Security-critical randomness comes from the Worker. The plug-in never mints a credential

**Verification: Inspection.** A placement rule about where a value is born, so it is verified by
reading which side calls `crypto.getRandomValues` rather than by running anything. **No mutation**,
for the same reason ADR-13 and ADR-21 carry `—`: the Lightroom client is Lua and
`scripts/mutation-check.py` runs the Vitest suite, which cannot see it.

**Every credential, nonce, token, code and session identifier MUST be generated by the Worker,
using `crypto.getRandomValues`.** The Lightroom Classic plug-in **MUST NOT** generate any value
whose security depends on being unpredictable.

**The plug-in MAY generate values that need only to be UNIQUE** — correlation identifiers,
temporary filenames — and **SHOULD** prefer a deterministic `LrDigest.SHA256` over inputs it
already holds, because a retry then produces the same value.
...
### Reopening requires ALL THREE, and each is checkable

1. **Adobe DOCUMENTS the generator in a published SDK reference**, naming the underlying source and
   not merely the function signature. Checkable against the archive in `vendor/`.
2. **AND a use case exists that genuinely requires client-side unpredictability** — meaning the
   ...
   walked in `docs/LRC-CLIENT-NOTES.md` and each one wanted server generation or uniqueness.
3. **AND that generator's disappearance or weakening fails loudly**, with a check that can detect it
   at runtime.

### The argument that MUST NOT reopen this

**"`LrUUID` exists and its output looks correct."** That is precisely what was measured on
2026-08-16, and it is what lost. Re-raising it re-litigates a closed question using the evidence
that closed it. **A future session finding `LrUUID`, seeing well-formed version-4 UUIDs, and
concluding the plug-in can now generate credentials is repeating this decision's losing argument
while believing it has found something new.**

**Nor does longevity reopen it.** A decade of sightings is evidence about *stability*, and the
objection is about *provenance*. Those are unrelated properties and only the first is what forum
history can demonstrate.
...
| `LrUUID.generateUUID` for credentials | **Real, measured, and refused.** Undocumented, so its source is uncommitted and its weakening would be silent. See ADR-23 |
