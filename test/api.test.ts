...
describe("ADR-09, nothing behind a session reaches a shared cache", () => {
	it("marks the 200 AND the 401, which is what middleware order gets wrong", async () => {
		...
		const ok = await authed("/v001/me");
		const denied = await SELF.fetch(`${API}/v001/me`);
...
describe("withdrawing a request", () => {
	const withdraw = (id: string, nsid = NSID) =>
		authed(`/v001/requests/${id}/withdraw`, { method: "POST" }, nsid);
...
describe("ADR-08, the queue view is where fail-polite becomes visible", () => {
	...
	const page = async (query = ""): Promise<Page> =>
		(await (await authed(`/v001/queue${query}`)).json()) as Page;
...
describe("ADR-17, pagination", () => {
	...
	it.each([0, 201])("rejects limit=%s", async (limit) => {
		expect((await authed(`/v001/queue?limit=${limit}`)).status).toBe(400);
	});

	it("refuses a cursor belonging to somebody else", async () => {
		const theirs = await seed(OTHER, "p1", "g1");
		expect((await authed(`/v001/queue?after=${theirs}`)).status).toBe(400);
	});
});
